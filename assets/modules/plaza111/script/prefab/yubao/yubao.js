...
glGame.baseclass.extend({
    ...
    onLoad() {
        ...
        this.isUpdate = false;
        // this.ReqDiscountCoinBalanceRecordList();
    ...
    registerEvent() {
        ...
        glGame.emitter.on(`${this.node.name}${MESSAGE.UI.ACTION_END}`, this.ReqDiscountCoinBalanceSummary, this);
    },
    // 销毁界面监听事件
    unRegisterEvent() {
    ...
    showLowesttips() {
        if (this.yubaoData.can_receive_profit <= 0 && this.yubaoData.can_receive_profit == 0) {
            let str = glGame.tips.YUBAO.NOREWARD
            glGame.panel.showErrorTip(str)
            return;
        } else {
            let str = glGame.tips.YUBAO.MINPROFIT.format(`${this.getFloat(this.yubaoData.min_receive_profit)}`);
            glGame.panel.showErrorTip(str)
            return;
        }
    },
    ...
    initMainUI() {
        ...
        this.lb_differenceexplain.string = `您还有 <color=#f4c404>${this.getFloat(this.yubaoData.unsatisfied_coin)}</color> 金币不满足打码量要求，未转化为有效余额！`;
        if (this.yubaoData.unsatisfied_coin <= 0) this.lb_differenceexplain.node.active = false;
        this.lb_yeares.string = `${(this.yubaoData.profit_percent / 100 * 365).toFixed(2)}%`
        let dayes = ((this.yubaoData.accounts_effective_coin * this.yubaoData.profit_percent / 10000).div(100)).toFixed(2);
        ...
        this.lb_rate.string = this.yubaoData.audit;
        // 界面 2
        ...
        this.lb_totales2.string = this.getFloat(this.yubaoData.all_produce_profit);
        // 界面 3
    ...
    setEarningsUI() {
        ...
        let begin = (this.earningsPage - 1) * this.earningsSize;
        if (begin > this.earningsList.length) return;
        let end = Math.min(this.earningsPage * this.earningsSize, this.earningsList.length);
        this.earnings_content.destroyAllChildren();
        this.earnings_content.removeAllChildren();
        this.lb_earningspage.string = `第 ${this.earningsPage}/${this.earningsMax} 页`
        for (let i = begin; i < end; i++) {
            let data = this.earningsList[i];
            let rightItem = cc.instantiate(this.earnings_item);
            rightItem.parent = this.earnings_content;
            ...
            rightItem.getChildByName("yearget").getComponent(cc.Label).string = `${(data.percent / 100 * 365).toFixed(2)}%`;
    ...
    setRecordUI() {
        ...
        let begin = (this.recordPage - 1) * this.recordSize;
        if (begin > this.recordList.length) return;
        let end = Math.min(this.recordPage * this.recordSize, this.recordList.length);
        this.record_content.destroyAllChildren();
        this.record_content.removeAllChildren();
        this.lb_recordpage.string = `第 ${this.recordPage}/${this.recordMax} 页`
        for (let i = begin; i < end; i++) {
            let data = this.recordList[i];
            let recordItem = cc.instantiate(this.record_item);
            recordItem.parent = this.record_content;
    ...
    // 收益记录显示
    ReqDiscountCoinBalanceRecordList() {
        if (this.earningsList.length <= this.earningsSize * (this.earningsPage - 1) && this.earningsPage <= this.earningsMax) {
            glGame.gameNet.send_msg('http.ReqDiscountCoinBalanceRecordList', { page: this.earningsPage, page_size: this.earningsSize }, (route, msg) => {
                this.earningsList = this.earningsList.concat(msg.list);
                this.earningsMax = msg.total_page;
                this.setEarningsUI();
            })
    ...
    //领取记录
    ReqDiscountCoinBalance() {
        if (this.recordList.length <= this.recordSize * (this.recordPage - 1) && this.recordPage <= this.recordMax||this.isUpdate) {
            glGame.gameNet.send_msg('http.ReqDiscountCoinBalance', { page: this.recordPage, page_size: this.recordSize }, (route, msg) => {
                this.recordList = this.recordList.concat(msg.result.list);
                this.recordMax = msg.result.total_page;
                this.isUpdate = false;
                this.setRecordUI();
            })
    ...
    ReqDiscountCoinBalanceApply() {
        ...
        glGame.gameNet.send_msg('http.ReqDiscountCoinBalanceApply', {}, (route, msg) => {
            this.isUpdate = true;
            if (this.yubaoData.mod == 1) {
                let strTitle = glGame.tips.YUBAO.CONGRATULATE;
                glGame.panel.showAwardBox(strTitle, [{ type: glGame.awardtype.COIN, value: this.getFloat(this.yubaoData.can_receive_profit) }]);
            } else if (this.yubaoData.mod == 2) {
                glGame.panel.showMsgBox("", glGame.tips.YUBAO.NOTICETIPS.format(`${this.getFloat(this.yubaoData.can_receive_profit)}`));
            }
            glGame.user.ReqRedDot();
            glGame.user.reqGetCoin();
            this.yubaoData = null;
            this.ReqDiscountCoinBalanceSummary();
            this.recordList = [];
        })
    },
    ...
    onClick(name, node) {
        switch (name) {
            case "btn_close": this.click_cancel(); break;
            case "btn_getrecord": this.btn_getrecordCB(); break;
            case "btn_require": this.btn_requireCB(); break;
            case "btn_closeget": this.btn_closegetCB(); break;
            case "btn_getearnings": this.btn_getearningsCB(); break;
            case "btn_pageup": this.btn_pageupCB(); break;
            case "btn_pagedown": this.btn_pagedownCB(); break;
            case "btn_up": this.btn_earningsup(); break;
            case "btn_next": this.btn_earningsnext(); break;
            case "yuebaoMain":
            case "earnings":
            case "getaward":
    ...
    hideAllPanel() {
        ...
        for (let i = 0; i < this.son_panel.childrenCount; i++) {
            this.son_panel.children[i].active = false;
        }
    },

    //显示某个界面。按名字来显示
    cutshowPanel(panelName) {
        this.hideAllPanel();
    ...
    btn_pageupCB() {
        this.recordPage--;
        if (this.recordPage < 1) this.recordPage = 1;
        else this.setRecordUI();
    },
    btn_pagedownCB() {
        this.recordPage++;
        if (this.recordPage > this.recordMax) this.recordPage = this.recordMax;
        else this.ReqDiscountCoinBalance();
    },
    btn_earningsup() {
        this.earningsPage--;
        if (this.earningsPage < 1) this.earningsPage = 1;
        else this.setEarningsUI();
    },
    btn_earningsnext() {
        this.earningsPage++;
        if (this.earningsPage > this.earningsMax) this.earningsPage = this.earningsMax;
        else this.ReqDiscountCoinBalanceRecordList();
    },
