const dzp_result = {
    1: "img_hongdian", 2: "img_heidian", 3: "img_hongdian", 4: "img_heidian", 5: "img_hongdian", 6: "img_heidian", 7: "img_hongdian", 8: "img_heidian", 9: "img_hongdian", 10: "img_heidian",
    11: "img_heidian", 12: "img_hongdian", 13: "img_heidian", 14: "img_hongdian", 15: "img_heidian", 16: "img_hongdian", 17: "img_heidian", 18: "img_hongdian", 19: "img_hongdian", 20: "img_heidian", 21: "img_hongdian",
    22: "img_heidian", 23: "img_hongdian", 24: "img_heidian", 25: "img_hongdian", 26: "img_heidian", 27: "img_hongdian", 28: "img_heidian", 29: "img_heidian", 30: "img_hongdian", 31: "img_heidian", 32: "img_hongdian",
    33: "img_heidian", 34: "img_hongdian", 35: "img_heidian", 36: "img_hongdian"
} 

const state_Arr = ["准备中", "下注中", "结算中"];
...
glGame.baseclass.extend({
    ...
    showClock() {
        for (let i = 0; i < this.content.children.length; i++) {
            ...
            if(process == 2) {
                lbClock.string = `${szState}:  ${remainTime}`;
                if(remainTime <= 3) {
    ...
    showRecord(node, record) {
        ...
        let nodeDot = node.getChildByName('node_dot')
        ...
        for (let i = index; i < record.length; i++) {
            ...
            let delay = cc.delayTime(Math.floor(count++/12) * 0.10);
            let fadeIn = cc.fadeIn(0.15);
            dotNode.runAction(cc.sequence(delay, fadeIn));
        }

        let percent = [];
        for (let i = 4; i >= 0; i--) {
            let change_string =  Math.round((record.length > 48 ? change[i] / 48 : change[i] / record.length) * 100);
            if(i==0 || i==2) {
                change_string =  100 - percent[4] - percent[i+1];
            }
            percent[i] = change_string;
            node.getChildByName("change").children[i].getComponent(cc.Label).string = `${change_string}%`
        }
    },
    ...
    click_help() {
        let panel = glGame.panel.showPanel(this.lttRule);
        panel.zIndex = 30;
    },

    click_record() {
        let panel = glGame.panel.showPanel(this.lttRecord);
        panel.zIndex = 30;
    },
