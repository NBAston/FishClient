...
window.startGame = function () {
    ...
    glGame.devVersion = "2020-12-19 19:01:51";
    // 进入房间是否请求验证的测试开关
    ...
    //系统旋转校验布尔值
    glGame.isSystemRota = true;

    //按钮粒子发射器 启动/关闭 全局开关, 需求：对于性能较低的手机可能需要关掉粒子效果
    glGame.isPariticle = true;

    // 记录进入游戏的分类
    glGame.isCategoryData = { typeIndex: null, platIndex: null }
    ...
    if (cc.sys.platform == cc.sys.IPHONE) {
        let phoneName = glGame.platform.getPhoneModel();
        window.isiPhoneX = (phoneName === "iPhone X"
            || phoneName === "iPhone XR" || phoneName === "iPhone XS" || phoneName === "iPhone XS Max"
            || phoneName === "iPhone 11" || phoneName === "iPhone 11 Pro" || phoneName === "iPhone 11 Pro Max" 
            || phoneName === "iPhone 12 mini" || phoneName === "iPhone 12" || phoneName === "iPhone 12 Pro"|| phoneName === "iPhone 12 Pro Max");
    }

    if (cc.sys.isNative) cc.game.setFrameRate(30);
    // 场景标记
    ...
    glGame.screenshot = require('ScreenShot');
    ...
    cc.sys.localStorage.setItem('isAutomaticGrabTag', false);
